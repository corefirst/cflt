...
3.  **Muscle Memory Training:** Learners are trained to treat the final Time Token as a "trigger" that retroactively validates the core. CFLT *hypothesizes* that, in the "Grammar Overlay" stage, the brain begins to *pre-echo* the time token into the core verb, transforming "I go... yesterday" into "I went... yesterday" via a subconscious feedback loop. This proposed "pre-echo" is a CFLT training mechanism to be tested, not an established finding; no cited source establishes that it reproduces how native speakers handle long-distance dependencies. The intended outcome is to build muscle memory for inflection without the planning cost.
...
CFLT's effectiveness claims are *motivated by* established parsing principles and bilingual production models, and operationalized as the projected metrics below. These are predictions to be tested, not measured results; the numeric magnitudes are CFLT project targets, not effect sizes derived from the cited literature.
...
- **CFLT Projection:** CFLT applies EIC to discourse-level slots as an *analogy*, not a derivation; Hawkins's metric is defined over syntactic constituent-recognition domains and does not by itself yield a discourse-slot ratio or a working-memory reduction factor. CFLT *hypothesizes* that fronting the Core lowers the "look-ahead" load on working memory, and the §6 latency test is designed to measure whether and by how much. Any specific magnitude (e.g., an EIC ratio near 100%, or a 2×–5× load reduction) is a CFLT target to be measured, not a value entailed by EIC.

### 5.2 Production Latency (Experimental Projections)
**Hashimoto, Yokoyama & Kawashima 2012** [[10.2174/1874347101206010062](https://doi.org/10.2174/1874347101206010062)] is a *literature survey* comparing reported activation patterns across SVO- and SOV-language sentence-comprehension studies; it reports cross-typological differences (including frontal regions) interpreted as compatible with differing processing and working-memory loads. It does **not** run a controlled non-canonical-vs-canonical (or Figure-Ground reversal) experiment, so it provides **indirect neurotypological motivation**, not a direct measurement that non-canonical orders elicit increased left-frontal activity. We treat its relevance to CFLT as an open question for a primary fMRI study to settle. (The LATL combinatory work of Bemis & Pylkkänen 2013 and Pylkkänen 2019 concerns minimal semantic composition; we do not cite it for stability of composition across word orders, which it does not establish.)
...
- **Hypothesis:** CFLT *predicts* that training reduces (and at the limit may eliminate) this specific restructuring delay by shifting the linearization decision to an automatic pre-verbal stage. This is a CFLT-specific empirical prediction to be tested in §6. [Levelt's (1989)](https://mitpress.mit.edu/9780262620895/speaking/) production architecture supplies the stage (the Conceptualizer/Formulator boundary) at which CFLT locates this intervention; Levelt does not himself predict that any scaffold eliminates restructuring delay.
