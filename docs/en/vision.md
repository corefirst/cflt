...
>
> Reasoning in real LLMs is stochastic (autoregressive sampling). Most magnitudes below are open empirical questions rather than measured outcomes; the one exception now measured is the distractor-heavy extraction-accuracy gain, where a five-model pilot reports **+22–44pp** (see [`foundations/llm.md`](foundations/llm.md) §3 and [`methodology/llm-part2-verification.md`](methodology/llm-part2-verification.md)).

### Why the CFLT Protocol is a Promising LLM Protocol:
- **Attention-prefix benefit (accuracy gain measured; mechanism open):** Placing the `[Core]` at the beginning of the sequence locates the salience anchor in the high-attention prefix region. A five-model pilot reports that Core-first ordering raises distractor-heavy extraction accuracy by **+22–44pp** ([`methodology/llm-part2-verification.md`](methodology/llm-part2-verification.md)) — preliminary support (small N, single dataset). Whether this stems specifically from **position-0 primacy** (the argument in [`foundations/llm.md`](foundations/llm.md) §2.3 — **not** the softmax-stability attention-sink artifact, Xiao et al. 2024, disclaimed there) rather than from separating the Core from distractors is not yet isolated, and position-0 attention mass is unmeasured (`llm.md` §2.4, §10.1).
