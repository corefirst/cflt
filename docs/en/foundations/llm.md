...
4.  **Non-monotonic position bias.** Pezeshkpour & Hruschka (2024) show that LLM accuracy on multiple-choice questions is sensitive to the *order of options* in non-monotonic ways — first-position is not uniformly advantaged; some models systematically prefer the last option, others the middle. Zheng et al. (2024) document similar non-monotonic position effects in LLM-as-judge evaluations. **This is a direct partial counter to CFLT's primacy-favors-Core-first claim**: if position-0 advantage is task-dependent and non-monotonic, CFLT's strict Slot-0 placement may not be optimal for all tasks. The CFLT pilot (`methodology/llm-part2-verification.md` §2.3) already shows this empirically — L4 multi-action cases on DeepSeek V4 Pro show CFLT *underperforming* control by 11pp, consistent with the Pezeshkpour & Hruschka finding that buried-decision contexts behave differently from distractor-noise contexts. **Open question**: is the L3-vs-L4 reversal a property of (a) the task type (distractor extraction vs. multi-option decision), (b) the model's intrinsic positional prior, or (c) the interaction? A pre-registered factorial design across ≥ 3 model families and ≥ 4 task types (extraction, MCQ, summarization, multi-step planning) is required.
