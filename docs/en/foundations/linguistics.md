...
1. At the **conceptual** level, Levelt (1989) establishes that production separates a pre-verbal message stage from grammatical/phonological formulation. CFLT *hypothesizes* that the salient event/state/copular/directive is the natural anchor of that pre-verbal message; Levelt does not establish that this anchor is cognitively prior or must be uttered first, so this is a CFLT proposal motivated by — not derived from — the architecture.
...
**Honest residual**: Halliday's *Role* (e.g., *acting as a teacher*) does not have a clean home and is treated case-by-case (often dissolving into either Copular-Core or a Space-as-domain reading). This is acknowledged as a boundary case in `methodology/slot-disambiguation.md` §8.
