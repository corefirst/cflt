...
**Status (2026-05 pilot).** The *task-accuracy* limb of this condition has now been tested across five frontier model families (GPT-5, Gemini 3 Flash, Qwen3.5-Plus, DeepSeek V4 Pro, Claude Sonnet 4.6) and **not** refuted: Core-first ordering raised distractor-heavy (L3) extraction accuracy by +22–44pp, with every surveyed model reaching 100% (see [`../methodology/llm-part2-verification.md`](../methodology/llm-part2-verification.md)). This is preliminary (small N, single dataset, authored prompts). The *attention-weight-concentration* limb remains untested, and the pilot does not isolate position-0 primacy from distractor-separation — the dedicated buried-decision (L4) probe was under-powered (four of five models already at ceiling in control; see `llm.md` §2.4). So P2's accuracy limb is preliminarily corroborated; its mechanism limb is open.
