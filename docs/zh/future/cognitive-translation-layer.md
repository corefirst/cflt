...
CTL 研究计划组织为**多阶段方案**。两端的底物已经独立部署并被验证，是这一研究范围**可信**的前提。下列阶段按**逻辑顺序**呈现；实际执行时间表取决于研究环境（独立研究、工业研究实验室、或研究生项目）与社区参与情况。各阶段**有意设计为解耦的**：任何阶段都可以由感兴趣的研究者**不依赖作者本人路径**地独立推进；针对单个子主张（CTL-1 至 CTL-5）的贡献也独立受欢迎。
