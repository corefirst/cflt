...
**核心优先语言理论 (Core-First Language Theory, CFLT)** 是一个面向跨语言通信与双语教育的统一理论框架。它提出一条话语层级 (discourse-level) 的核心原则——*话语的认知核心，也是其在任何语言中被普遍优先排列的线性位置*——并由此推导出一种可教学、可被 AI 支持的方法（**核心优先语言方法 (Core-First Language Method, CFLM)**），用以最小化任意两种自然语言之间的认知摩擦。
...
| 层级 | 名称 | 角色 |
|---|---|---|
| 项目 / 品牌 | **CoreFirst** (corefirst.world) | 涵盖理论、方法与应用的伞品牌 |
| 理论 | **CFLT — 核心优先语言理论** | 学术主张，可被引用的框架 |
| 方法 | **CFLM — 核心优先语言方法** | 教学层面的实现 |

项目目前以同名产品 *CoreFirst* 单一交付，是当前阶段的有意单品牌策略。若未来出现其他产品分支，可能引入子品牌。
...
> **重要适用范围说明。** 四元素规范序列定义的是 CFLM 的 **无标记默认 (unmarked default)** 形式 —— 即流利说话者在没有特殊修辞目的时所产出的形式。它**并不**禁止成熟流利度所需的有标记偏离 (marked deviation)：话题化 (topicalization)、前置 (fronting)、分裂句 (clefts)、句末重置 (end-weight repackaging) 等。每一种自然语言都对同一命题内容拥有多种表达形式，CFLM 通过将自身定位为 *基线 (baseline)* 来兼容这一现实——有意的有标记偏离将在掌握基线之后才被引入。完整的无标记/有标记区分参见 [`foundations/core-concept.md`](foundations/core-concept.md) §7。

### 3.2 演示 CFLT 转换

CFLM 中的 Core 是一个**显著性锚点 (salience anchor)**，而不是动词（参见 [`foundations/core-concept.md`](foundations/core-concept.md)）。下面四个例子分别展示该协议所兼容的四种 Core 类型。

#### 例 1 —— 动作型 Core
**L1（中文，重语境顺序）：** *昨天下雨，我在家没出去。* —— 时间 → 原因 → 结果
**CFLT 重构形式：** *我没出去，因为下雨，在家，昨天。* —— Core → 原因 → 空间 → 时间
**英语 (CFLM-L2)：** *I didn't go out, because it rained, at home, yesterday.*

#### 例 2 —— 身份／描述型 Core
**L1（中文）：** *那个穿红衣服的女孩是我妹妹。* —— 修饰语堆叠的名词短语 → 身份
**CFLT 重构形式：** *那个女孩是我妹妹，穿着红衣服，在照片里，去年夏天。* —— Core（身份）→ 描述 → 空间 → 时间
**英语 (CFLM-L2)：** *That girl is my sister, wearing a red dress, in the photo, from last summer.*

#### 例 3 —— 状态型 Core
**L1（中文）：** *开了一下午会，我在办公室都累瘫了。* —— 原因 → 空间 → 状态
**CFLT 重构形式：** *我累瘫了，因为开了一下午会，在办公室，刚才。* —— Core（状态）→ 原因 → 空间 → 时间
**英语 (CFLM-L2)：** *I'm exhausted, because of the meeting, in the office, just now.*

#### 例 4 —— 请求／言语行为型 Core
**L1（中文）：** *现在能在桌上帮我递一下盐吗？* —— 时间 → 空间 → 请求
**CFLT 重构形式：** *能帮我递一下盐吗，请，在桌上，现在？* —— Core（请求）→ 礼貌标记 → 空间 → 时间
**英语 (CFLM-L2)：** *Could you pass the salt, please, on the table, now?*
...
### 阶段 3：文化精修（高级模块）
当通过 CFLM 实现功能性流利度后，AI 引入文化特有的习语、隐喻与高级文体细微差别。
...
#### B. 时态 → 语义时间词元
为解决英语时态的复杂性，CFLM 采用"时间词元 (Time Tokens)"。
...
*   *CFLM 逻辑：* "That man is my boss, [描述] he is standing there."

### 7.3 产品逻辑：语义优先，语法其次
1.  **语义核心 (Semantic Core)：** 优先保证 CFLT 逻辑的正确排序。
2.  **语法叠加 (Grammar Overlay)：** AI 作为非侵入式的"自动校正"插件，将用户的 CFLM 输出精修为地道的 L2，而不打断思维流动。
...
### 8.1 跨年龄适配
CFLM 作为各年龄段教育内容的基础：
- **早期学习者：** 聚焦"视觉化 CFLM (Visual CFLM)"，使用动画图标与约 500 个语义原语的受限集合，构建直觉性的概念-逻辑映射。
- **成人学习者：** 聚焦"效率化 CFLM (Efficiency CFLM)"，针对专业场景使用行业特定词元与复杂逻辑连接词。

### 8.2 行业特化模块："IT 英语"案例
CFLM 框架特别适合技术沟通。在 **IT 行业**，"动作-结果优先"的逻辑与工程文档及协作编码完美契合。
- **示例词元：** *deploy, refactor, debug, latency, endpoint, scalability.*
- **逻辑映射：** 不再说 "If we have high latency, we should refactor the code"，而是 CFLM 推荐："**Refactor the code**, because of **high latency**, in the **backend**, now."
- **收益：** IT 专业人员可以跨文化即时、准确地传达关键技术决策。

同样的模块化词汇注入模式可扩展到医疗、金融、技术、酒店服务等领域，而无需更改底层认知协议。

### 8.3 多模态交付（音视频合成）
- **音频优先 (Audio-Primary)：** 优先提供语音输出，韵律与重音模式强调 CFLM 块的 `[Core Action]`。
- **视觉支撑 (Visual-Support)：** AI 生成的图像或短片为核心概念提供即时视觉反馈，绕过母语翻译的需求。

### 8.4 自动化课件生成
借助 LLM，CFLM 框架可基于简单提示词（如"为 8 岁儿童设计的医院场景"）自主生成完整课程。这确保所有教育内容都与"Core First"排序原则保持一致。
...
### 9.1 拼音-到-IPA 桥梁
对于亚洲学习者（尤其是中文背景），CFLM 利用其对**拼音 (Pinyin)** 的既有掌握来加速发音习得。
...
语言学习是一项体能技能。通过识别 L1 与 L2 之间的"肌肉重叠 (Muscle Overlap)"，CFLM 减少学习"新"音的认知阻力，将其视为熟悉动作的变体。这与 Core-First 排序原则互补：§3 减少**句法摩擦**，§9 减少**发音摩擦**。两者都源于同一第一性原理思路——利用学习者大脑与身体已经编码的内容。
...
*由 Chinese2English AI 项目创建，2026。项目主页：corefirst.world。*
