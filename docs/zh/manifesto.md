...
> **如何读这些重构。** 下面每条 *CFLT 重构* 展示的是 **Core-first 的中介语（思维）层**，不是源语言的地道表层输出。目标语的落地（*英语 (CFLT-L2)* 行）按该语言**母语**习惯实现；尤其**尾巴序是语言相对的** —— 此处 *理由 → 空间 → 时间* 只是**一种示例排法，非规定**（参见 [`methodology/empirical-agenda.md`](methodology/empirical-agenda.md) §1.2）。前置框架状语的语言（如中文）会相应重排尾巴。
...
**CFLT 优势：** 在所有四种核心类型中，一旦学习者在母语思维中采用了核心优先排序，生成目标语言就**在很大程度上**变成了一项**令牌置换**练习，而不是结构重组 —— 其中场景框架的**尾巴按目标语言的母语习惯排序**，而非单一固定序列。不变的是核心优先；填充位置 0 的内容随说话者的意图而变化。
