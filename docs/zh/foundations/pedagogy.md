...
```
*学习者周围的三个嵌套区；ZPD 是中间带（依据 Vygotsky 1978）。连线为无向 —— 表示区间边界，而非进阶顺序。*
...
**CFLT *与*弱式 TBLT（Ellis 2003）和 VanPatten 加工教学兼容。** Ellis 把显式形式聚焦与任务实践视为互补；VanPatten 的加工教学在交际使用前显式训练学习者的形式-意义映射。CFLT 同时操作化两者：协议是显式支架，课件是任务交付。

**CFLT 课件生成器作为弱式 TBLT 引擎：** 生成器接收输入 `topic`（主题）、`industry_context`（行业背景）、`age_group`（年龄组）、`difficulty_level`（难度级别），并生成一系列基于场景的任务。每个任务都是一个符合协议的交际场景，嵌入了词汇重点和视听情境。IT 英语模块（宣言 §8.2）是规范案例：诸如"部署服务、调试延迟问题、重构模块"等任务携带行业适用词汇，而协议提供结构骨架。
...
对固定序列协议的一个常见批评是，它们可能导致“语言僵化”或“模板僵化”。为了解决这个问题，CFLT 结合了 **Skehan 的权衡假设 (Trade-off Hypothesis)** (1998)。
...
**CFLT 的战略选择：**
- **在 A1–B1 级别**，CFLT 通过固定序列优先考虑**流利度**和**准确性**，从而降低“复杂度”（规划）负荷。这使学习者能够尽早获得交际成功。
- **在 B2+ 级别**，目标发生转移。学习者已经实现了无标记序列的自动化，现在必须重新引入**复杂度**。

### 11.2 去支架化 (脚手架撤除)
精通的定义不是严格遵守协议，而是**有意识地**偏离协议的能力。CFLT 的高级模块（第三阶段）显式教授：
- **主题化 (Topicalization):** 将 [时间] 或 [空间] 移至句首以进行主题强调。
- **末尾焦点 (End-Focus):** 重组核心以将新信息置于句末。
- **分裂句 (Clefting):** 用于聚焦和对比。

在这种观点下，CFLT 协议是一个**生存脚手架**，最终被“淹没”到学习者的潜意识中，成为他们可以为了修辞细微差别而偏离的默认逻辑。
