...
条件熵 $H(\text{rest} \mid \text{core})$ 显著低于 $H(\text{rest} \mid \text{context})$，这意味着听者/模型能够实现**更早的歧义消解**。
...
通过将核心置于位置 0，CFLT 协议最小化了主句的 CRD。听者立即识别出“锚定”成分，实现了极高的 IC-to-word 比率（在核心识别阶段通常接近 100%）。这减少了解析器所需的**前瞻缓冲区 (look-ahead buffer)**，直接降低了认知负荷。
...
## 5. 最优编码 (Source Coding Theorem)

香农的源信道编码定理 (1948) 和哈夫曼编码 (1952) 告诉我们，**频繁使用或高信息量的项目应该占据简短、易于访问的编码位置**。

将其转化为子句线性化：
- 核心动作是**消解说话者意图歧义的最短路径**。
- 将其放在位置 1（最易访问的位置）可以最小化听者的预期查找成本。

这类似于将最频繁访问的指令放在函数的入口点 —— 也就是“前置分发（front-load the dispatch）”的工程原则。
...
1. **UID 张力：** 如 §3 所述，母语人士的地道英语旨在追求平坦的信息密度（通常通过末尾权重实现）；严格的 CFLT 会产生起伏的信息密度。地道的润色必须在表面阶段进行。
...
1. **最优槽位数量：** 为什么是四个？认知成本函数能否证明恰好是四个槽位，还是这只是一个基于实证的启发式方法？
2. **槽位顺序证明：** 在任何自然成本函数下，`核心 → 原因 → 地点 → 时间` 是否可证明是最优的，还是它只是多个局部最优解之一？
