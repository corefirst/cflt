...
其中 $\text{root}(G)$ 是核心（Core）。通过在**所调查的类型范围内**固定这一函数（“在 CFLT 所调查类型范围内”这一范围的形式重述见下文 §9），CFLT **旨在**将翻译的**计算复杂度**从结构转换（structural transformation）降低为词汇替换（lexical substitution）。关于支持“在所调查类型范围内使用同一压平函数”这一主张的类型学证据——涵盖四个语系的五种语言，并对每种核心内部组装机制给出参考语法引用——见 [`core-concept.md`](./core-concept.md) §2.5。

### 1.1 系属核心与指令核心建模
为了证明 CFLT 不仅仅是“动词领先”，我们将核心建模为语义 DAG 中**显著性最高（highest-salience）的节点**，无论其词性如何。

- **系属核心（系表结构/静态）：** 在 "That girl is my sister" 中，语义根是恒等关系 $\{A=B\}$。线性化保持为 $L([A=B], \text{modifier}) = [A=B, \text{modifier}]$。其数学优势在于在处理任何描述性属性之前，立即确定了**参考框架**。
- **指令核心（施事力）：** 在请求中，施事算子（如 `REQUEST(action)`）是根。CFLT 将其线性化为 $[\text{算子}, \text{动作}, \text{语境}]$。这最小化了**施事歧义** —— 听者在听到具体任务（“什么”）之前，就已经知道他们为什么要听（“为什么”）。
...
在 **CFLT 协议**中，核心占据 $t_1$ 位置。CFLT *将*核心*定义*为显著性锚点，并*假设*它是信息量最高 / 约束力最强的成分；在该假设下，它对后续所有槽位的概率分布提供了最强的约束。（核心始终是最具信息量或最具约束力的成分，是一项待测量的 CFLT 预测，而非既定前提。）
...
CFLT *预测*条件熵 $H(\text{rest} \mid \text{core})$ 低于 $H(\text{rest} \mid \text{某修饰语})$，若成立则意味着听者/模型能够实现**更早的歧义消解**，即使总联合熵不变。该不等式是一项**可测量、可预注册的 CFLT 假设** —— 它不是定理，也不是被引用的信息论结论（链式法则与熵并不能确立它，且它无需对每个 token 或语境都成立）。这正是 Core-first 在信息论意义上获得*动机*的准确含义：它指向一个待测量的量，而非既定的收益。
...
- **对于母语者**，UID 与流利话语中信息密度大致恒定的倾向相关联。英语中的末尾聚焦（end-focus）和末尾重量（end-weight）确实将沉重的名词短语（NPs）置于右侧；但它们是否*正是为了平滑密度*，是一种带 UID 色彩的解读，而非 Quirk 等（1985）或 FSP 所确立的结论 —— 末尾重量与 UID 平坦度之间的联系需要直接测量。
- **对于 L2 学习者和 AI 智能体**，UID 是一族*生产端*假设。CFLT *推测*（这并不由 UID 推出）流利的 UID 式信息打包所预设的全局规划超出了新手学习者的能力，因此*提议*转而优化**理解端**的解析确定性——通过前置核心，并接受一个不那么平坦的密度曲线。学习者无法进行全局规划，以及理解端前置对他们或对 AI 是正确的优化目标，都是待检验的 CFLT 主张，而非 UID 的推论。

因此，CFLT 并不是对 UID 的反驳；CFLT *提议*一个它假设适用于不同说话者群体的**不同优化目标**。
...
1. **UID 张力：** 如 §4 所述，带 UID 色彩的论述将母语人士的地道英语与趋向更平坦信息密度的倾向相关联（常与末尾权重挂钩），相对而言严格的 CFLT 可能产生更起伏的信息密度。"母语英语旨在追求平坦密度"与"CFLT 起伏"这一对照都是需要**直接测量**的主张，而非 Quirk 等或 FSP 所确立的事实；若成立，则在表面阶段施加地道润色是所提议的补救办法。
