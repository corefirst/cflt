...
1. 在**概念**层面，Levelt (1989) 确立了言语生成将前言语信息阶段与语法/音系表述区分开来。CFLT *假设*显著的事件/状态/系属/指令是该前言语信息的自然锚点；Levelt 并未确立这一锚点在认知上优先或必须最先说出，因此这是一个受该架构启发、而非由其推导出的 CFLT 提案。
...
**诚实的残余**：Halliday 的 *Role*（如 *作为老师做事*）没有干净的归宿，案例处理（通常分解为系属核心或空间作 domain 解读）。这在 `methodology/slot-disambiguation.md` §8 中作为边界案例承认。
