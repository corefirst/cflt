...
这些是**工程论证**，不是推导。Core → Reason → Time → Space 的协议也可由竞争性论证支持。话语分析文献中最有力的此类对手是 **"时间锚点优先"** 立场：Reinhart (1984，《叙事文本时间组织中的格式塔感知原则》) 论证，在叙事话语中时间位置是首要结构锚点 —— 读者按*何时*而非*为何*组织事件序列。在该论证下，叙事体裁 L2 产出可以为 Core → Time → Reason → Space 顺序辩护。CFLT 选择 R-S-T 是因为它的三条理由在项目的两个目标使用场景（L2 会话/说明文教学、LLM 提示词稳定性）中综合上胜过叙事-时间锚点；叙事性 L2 产出是一个已被识别的边界场景，R-T-S 备选方案可能在那里表现更佳，这一选择仍然是**开放的优化问题**（参见 `mathematics.md` §12.2 与 `methodology/evaluation-metrics.md` 中的实证协议）。
