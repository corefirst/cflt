...
核心由以下四组构成：
1.  **谓词**（动作、身份、状态或请求）。
2.  由谓词授权的**价位绑定参与者**（主语、宾语、工具、受益者、接受者、伴随对象）。依据 Levin & Rappaport Hovav (2005) 事件结构理论和 [`core-concept.md`](./core-concept.md) §2.2 的替换测试：当改变某个论元会改变事件身份时，该论元属于*价位绑定*——只要动词授权它，工具和受益者就是价位绑定参与者，而不是"内部修饰语"。
3.  **核内 / 核心层方式状语**（动作方式、运动方式）。
4.  以事件为论元的**辖域内算子**：否定、情态、时体、程度。

任何回答 **"如何？"**、**"用什么？"**、**"为/给谁？"**、**"与谁一起？"**、**"以何种情态？"**、**"是否被否定？"** 或 **"到什么程度？"** 的修饰语都融合在核心内部（属于上面第 2–4 组）。任何回答 **"为什么？"**、**"在哪里？"** 或 **"何时？"** 的修饰语都被剥离到槽位 1、2 或 3。
...
- **规则：** 程度或方式副词（如 *heavily, slightly, quickly*）描述的是事件的内部品质，而非其外部框架。
