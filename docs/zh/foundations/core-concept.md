...
在为 CFLT 术语审查所查阅的文献中（完整书目见内部调查 [`/docs/_research/importance-terminology-survey.md`](../../_research/importance-terminology-survey.md)），**"重要 (important)"一词从未作为技术定义术语出现**于语言学、认知科学、NLP 可解释性或厂商提示词工程文档中。凡是接近该含义之处，文献一律使用 *焦点 (focus)*、*显著性 (salience)*、*突显 (prominence)*、*主题 / 话题 (theme / topic)*、*图形 (figure)* 或 *高惊奇度 (high-surprisal)*——每一个术语都将评估性重要性与结构性锚定分开。采用"重要"作为 CFLT 术语，会把主流信息结构理论刻意区分的四个认知独立维度（焦点 / 显著性 / 可及性 / 惊奇度）混为一谈（Krifka 2008；Gundel, Hedberg & Zacharski 1993）。
...
CFLT 的位置 0 单元包含谓词、其**价位绑定参与者**（主语、宾语、工具、受益者、接受者）以及**方式状语**。按 RRG 分层，这对应 RRG 的 *Core* 层加上 *Periphery* 的一小部分（方式）。CFLT 中"核心"的术语选择因此是有意为之——它继承自 RRG 的 Core 层，而*非* RRG 的 Nucleus。若读者默默地把 CFLT 的核心等同于 RRG 的 Nucleus，将错误地得出"论元位于场景框架中"的结论。
...
**普遍主张**：否定、情态和体态始终**辖域覆盖事件核**，因此存在于槽位 0 内。即使它们在形态上可以与动词分离（如英语情态助动词、阿拉伯语 *lam*），也绝不会被归入场景框架。这与 Cinque (1999) 的发现一致：语气、情态和体态占据比词汇动词更高、但比 CP 更低的功能投射——该区域对应 CFLT 的 Core，而非场景框架。
...
2. **注意力汇点（Xiao et al. 2023，*Efficient Streaming LLMs with Attention Sinks*）。** 序列前几个 token 积累了不成比例的注意力质量——这是 softmax 稳定性的副作用，而非语义优先化，但其**结果**是：放在前缀处的信号在长上下文中得以更可靠地保留。注意：注意力汇点是架构层面的现象，有别于 `llm.md` §1.1 中的认知首因效应；CFLT 从两者中获益，但它们是独立机制。
...
2. **稳定的注意力锚点。** LLM 最关注位置 0；协议确保位置 0 始终是最重要的词（见 `llm.md` §2）。
...
- **Xiao et al. (2023) "Attention Sinks"** —— 将首 token 注意力集中显示为 softmax 稳定性现象。提供了 CFLT 位置 0 获得不成比例模型注意力的*机制*。
...
flowchart TD
    ...
    E -->|"修饰语改变 → 事件身份改变"| F[归入 Core<br/>方式 / 工具 / 受益者 /<br/>伴随 / 情态 / 否定 / 体态]
