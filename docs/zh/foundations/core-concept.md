...
|---|---|---|
| **事件 (Event)** | *I didn't go out*, because... | 事件 / 状态的变化 |
| **系属 (Copular)** | *That girl is my sister*, wearing... | 分类 / 命名 |
| **状态 (State)** | *I'm exhausted*, because... | 情况 / 品质 |
| **指令 (Directive)** | *Could you pass the salt*, please... | 言语行为 / 期望的结果 |
...
| 层级 | 内容 | 听者所问的问题 | 位置 |
|---|---|---|---|
| **第一层：事件核** | 谓词（事件 / 状态 / 系属 / 指令）+ 价位绑定参与者（主语、宾语、工具、受益者、接受者、伴随对象）+ 核内 / 核心层方式状语 + 辖域内算子（否定、情态、时体、程度）——参见 §9 的规范化枚举 | *发生了什么？*（含 谁、对谁、用什么、怎样、以何情态、是否否定、内部时点） | 槽位 0（核心） |
...
|---|---|---|---|
| **L1: 协议** | 核心位于位置 0；场景框架的尾巴**按目标语言的母语序**组装（示例默认：理由 → 空间 → 时间） | **核心优先 = 项目标准的无标记默认；尾巴序为语言相对 / 开放 —— 并非描述性普遍法则** | 没有任何语言享有特权。协议是语言无关的。 |
...
§2 中的分类（事件 / 状态 / 系属 / 指令）是 CFLT *自行定义*的、对说话者意图的聚合——而非任何单一引用来源所推导出的自然类别。本小节为每种类型给出形式内部结构——谓词、参与者，以及与既有谓词类型学和言语行为理论的关系。"理论锚点"一列列出的是**为这些区分提供动机的理论近邻**；所引来源（Vendler、Dowty、Carlson、Maienborn、Levin & Rappaport Hovav、Hengeveld、Higgins、Kennedy、Searle、Sadock & Zwicky、Brown & Levinson）提供分析词汇并为划分提供动机，但**并不**定义 CFLT 的四种核心类型，也不定义其位置 0 的安置。这是 Logic-Transformer 风格实现的权威参照。

| 核心类型 | 谓词签名 | 必要参与者 | Core 内可选项 | 理论近邻（提供动机，非定义） |
|---|---|---|---|---|
| **事件** | Λx.Λy. *event*(x,y, ...) — 价位 ≥ 1 的事件性谓词 | 施事 / 受事 (+ 谓词允许时的工具、受益者、接受者) | 方式副词；情态与否定算子；体态 | Vendler (1957) *event-types*；Dowty (1979) *event structure*；Levin & Rappaport Hovav (2005) |
| **系属** | Λx. *be*(x, P) — 系词谓述，其中 P 为属性/范畴/实体 | 主语 (载体)；系词补足语 (被谓述的属性/身份) | 情态与否定；补足语上的程度修饰语 | Hengeveld (1992) *non-verbal predication*；Higgins (1979) 系词子句分类 (predicational / specificational / identificational / identity) |
...
| **指令** | DIR(s, h, P) — 指令性言语行为，其中 s = 说话者，h = 听者，P = 请求的命题内容 | 说话者 (隐含)；受话者 (隐含或呼格)；请求的行为 (嵌入谓词) | 礼貌标记；情态叠加 (*could you, would you, might you*) | Searle (1969, 1975) *directives*；Sadock & Zwicky (1985) *speech-act typology*；Brown & Levinson (1987) *politeness-as-redress* |

**为什么这在操作上很重要。** 实现者（LLM 提示词设计者、教学材料作者、Logic Transformer 引擎）必须知道**什么填入槽位 0**，否则协议就是欠定的。上述签名解决了四种常规歧义：

1. **系属核心携带系词。** *"That girl is my sister"* —— 系词 *is* 与补足语 *my sister* 一起属于槽位 0，而非作为独立谓词。Hengeveld (1992) 的类型学证实，系词结构中的谓述元素是补足语与系词一起；系词携带时态 / 一致 / 极性，补足语提供谓述内容。CFLT 将**整个系词谓语** (系词 + 补足语) 置于位置 0。

2. **指令核心前景化指令行为，而非被请求的裸动作。** *"Could you pass the salt"* —— 槽位 0 是完整的指令谓词 *could you pass the salt* (言语行为力 + 嵌入命题内容)，**而非**抽取出来的 *pass-the-salt* 动作。Searle (1975) 的分类为"将言语行为力视为定义言语行为的要素"提供了动机；而*整个*指令归入槽位 0，是 CFLT 的槽位安置决定，并非 Searle 所确立的结论。若学习者将 *"Could you pass the salt"* 简化为 *"pass salt"*，就丢弃了区分礼貌请求与直白命令的礼貌 / 情态标记——两者都是指令，但语域本身是核心的一部分。
...
**综合**：在这三个文献领域所测试的模型家族与任务中，三种不同的行为模式（长上下文中的位置偏差、softmax 稳定性注意力汇点、上下文学习顺序敏感性）各自都与这一操作倾向相容：**对所测试的仅解码器 LLM、在所测试的任务族上，放在最前面的内容可能具有不成比例的重要性**。这些是处于句子以上尺度的彼此独立的现象，因此它们*启发*而非*确证* CFLT 的句级协议。**句级的直接证据如今来自 CFLT 自己的 Part II 中试**，而非这些外部文献：在五个前沿模型族上，Core 前置相比自然的"理由在前"语序，把干扰项密集（L3）的抽取准确率提升 +22–44pp，并把每个被测模型在该条件下拉到 100%（小样本、单数据集、作者自撰提示词的中试——初步而非已确立；见 [`../methodology/llm-part2-verification.md`](../methodology/llm-part2-verification.md)）。该中试证实的是 P2 的*任务准确率*支；它**并未**隔离*位置 0 首因*作为机制——专门用于隔离首因的"埋藏决策"探针（L4）欠功率（五个模型中有四个在对照组已处于天花板），故目前该增益更应被描述为"Core 前置带来的抗干扰性"，而非已确证的首因（见 `llm.md` §2.4）。CFLT 的协议层主张因此不是关于自然语言的类型学普遍性；它是一条 **其任务准确率支现已获初步支持、其机制仍由 CFLT 持续对照 Transformer 行为的可测属性加以检验的生成规则**。（任何关于人类认知的理据——例如序列位置效应或双语控制成本——是一条*独立*的动机线索，在 §8.5 的 P1/P3 与 `neuroscience.md` 中讨论；LLM 证据与人类证据保持分离，**不是**同一共享机制。）关于 Liu et al. 2023 的范围蔓延警告（文档量级发现 → 句子量级外推）在 `llm.md` §3 中有明确处理。这些 LLM 侧现象的完整讨论在 `llm.md` §2–§3（句级抽取证据见 Part II pilot，[`../methodology/llm-part2-verification.md`](../methodology/llm-part2-verification.md)）；本节是认知协议侧摘要。
...
| "CFLT 仅适用于动作句。" | CFLT 容纳四种核心类型（事件、状态、系属、指令）。协议是统一的；填充位置 0 的内容各异。 |
...
**状态（2026-05 中试）。** 本条件的*任务准确率*支现已在五个前沿模型族（GPT-5、Gemini 3 Flash、Qwen3.5-Plus、DeepSeek V4 Pro、Claude Sonnet 4.6）上测试，且**未**被推翻：Core 前置把干扰项密集（L3）的抽取准确率提升 +22–44pp，每个被测模型均达到 100%（见 [`../methodology/llm-part2-verification.md`](../methodology/llm-part2-verification.md)）。这是初步结果（小样本、单数据集、作者自撰提示词）。*注意力权重集中*支仍未测试，且该中试未能把位置 0 首因与"干扰项分离"区分开——专门用于隔离首因的"埋藏决策"探针（L4）欠功率（五个模型中有四个在对照组已处于天花板；见 `llm.md` §2.4）。因此 P2 的准确率支已获初步证实，其机制支仍开放。
...
> **核心是事件核 —— 谓词，连同那些与事件本身不可分割的所有参与者和辖域内算子。** 形式定义：它是最小的成分——当被单独说出时，能够识别说话者的主要意图 (事件、状态、系属或指令)。
>
...
> 2.  **步骤 0.2 谓词：** `[Event/State/Copular/Directive]` (锚点本身：动词、系词 + 补足语、静态中心词，或言语行为动词)
...
- [`mathematics.md`](./mathematics.md) §1.1 — 动作动词之外的系属 / 指令 / 状态核心的形式化建模。
- [`llm.md`](./llm.md) §2.4 — 非事件核心 (系属、指令) 如何与高注意力前缀区互动 (首因与汇点联合)。
