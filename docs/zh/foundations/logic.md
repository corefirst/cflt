...
**CFLT 从这一传统中借鉴了什么：**
1. **歧义消解的顺序。** 一阶逻辑把谓词置于首位是一种符号约定；CFLT *假设*（这是一项待检验的预测，而非一阶逻辑的结论）在自然语言中首先识别谓词同样能减少后续项的加工歧义。“吃（Eat）”很可能将参数限制为可食用的东西；“去（Go）”则限制为地点或路径。
2. **“中心词（Head）”地位。** 谓词是表达式的逻辑中心词。

**CFLT *没有*借鉴什么：**
- **符号形式** $P(a, b, c)$。CFLT 产生的是 "I went out, because it rained, at home, yesterday." —— 具有结构化顺序的可理解英语。
- **将“核心”简化为“谓词符号”。** 系动词结构（"That girl is my sister"）在动作动词意义上没有明显的“谓词”，但 CFLT *为其指派*了一个核心（身份断言）。在这里哪个成分算作核心是一种项目内部的识别、可能随语篇而变，而非从逻辑推导出的事实；CFLT 可以处理这种情况，而单纯的谓词逻辑则不行。
...
**关于普适性的说明：** Lambda 演算与类型无关 —— 函数 $\lambda e.\,\text{COMMIT}(e)$ 可以包裹一个事件、一个状态、一个系属谓述或一个指令。这种普适性正是函数应用比喻能自然延伸到 CFLT 四种核心类型的原因。
...
**CFLT 作为一种 CCG 类比：** CFLT 并没有发明一种新的组合语法，并且 CFLT 的槽位**尚未**被证明是 CCG 范畴、其表面语序也未被证明是一种 CCG 派生。我们仅把 CCG 当作类比，用以说明"从灵活空间中确定一种标准调度"这一思想：
1. **核心**扮演类似于主要算子（functor）的角色。
2. 它被理解为以固定的、可预测的方向“寻找”它的参数。

其激励性直觉是：固定单一派生路径或许能够回避那种使 CCG 解析复杂化的“伪歧义（spurious ambiguity）”。但 CFLT 协议是否真的在任何形式化 CCG 意义上减少了伪歧义，是一个待检验的 CFLT 特定问题，而非由 CCG 推出的结论；该类比作为面向 AI 智能体的桥梁提出，而非作为证明。
...
|---|---|---|
| **事件 (Event)** | 陈述类 (Representative/assertive) | "I went out" —— 断言一个事件 |
| **系属 (Copular)** | 陈述类 (Representative/assertive) | "That girl is my sister" —— 断言一个分类 |
| **状态 (State)** | 表达类 *或* 陈述类 | "I'm exhausted" —— 表达/断言一种状况 |
| **指令 (Directive)** | 指令类 (Directive，与 Searle 指令类重合) | "Could you pass the salt" —— 指示听者采取行动 |
...
> **关于承诺类 (Commissives) 与四类压缩的说明。** 上述映射省略了 Searle 五类中的两类 —— **承诺类**（promises，如 *"我明天会给你打电话"*；*"我保证会来"*) 和 **宣告类**（上文提到的专门施事范畴）。在 CFLT 中，承诺类实现为**事件核心的一个子类**：谓词是一个承诺动作（*保证、承诺、确保、会做*），嵌入的命题内容是被承诺的动作 —— 因此 *"我保证会来"* 解析为核心 = *我保证会来*。宣告类，在窄义施事范畴之外出现时，是系属核心的一个有标子类（通常是带规定约束力的系词谓语）。因此 CFLT 的四分分类法是对 Searle 五类行为的**有意进行的教学/操作性聚合** —— 这与上文关于宣告类的注记一致 —— 并非含蓄地否认承诺类和宣告类的存在。记录这一压缩是为了防止读者寻找缺失的第五种核心类型。

将 Searle 五类压缩为四类，以及"首先识别**施事力（illocutionary force）**对听者有益"这一主张，都是 **CFLT 的操作性选择与待检验的预测** —— 它们并非从 Searle 或 Sadock & Zwicky 推导而来；后者区分施事力与内容，但并未规定任何线性化顺序。带此 caveat，CFLT *提议*将施事力首先置出，其假设是这能顺应听者对于确认自己是被告知、被请求还是被称呼的需求。
...
**CFLT 作为一种关联最大化策略：** CFLT *预测*将核心置于位置 0 会把最高效果的 Token 放在听者注意力最集中的地方，从而使听者（或 LLM，见 `llm.md`）可以从核心开始进行推理计算，而不需要在修饰语中漫长等待以发现话语的主题。（注意：位置 0 持有最高效果内容、且这一点可延伸到 LLM 注意力，是 **CFLT 的预测，而非关联理论的主张**。关联理论并未将首个 Token 认定为最高效果内容。它**激励**前置高效果内容，但并不**蕴含**位置 0 本身——它权衡的是*总*认知效果与*总*努力，且允许后置信息也最优相关。所以关联理论与 Core-first **相容**，非**证明**。）
...
方式准则 —— *“要有序”* —— 是在**激励**拥有可预测的线性化。CFLT 提供了一个标准顺序，它**满足**了有序准则——但**任何**固定约定都同样满足它（Grice 自己的有序例子是时间序）。所以方式准则**激励一个固定协议；它本身并不蕴含 CFLT 的*特定*顺序**（Core 前置、R→S→T）——那个特定顺序由别处的认知/加工论证支撑，不是由方式准则。带此 caveat，该协议**符合格赖斯原则**：母语人士用语境敏感启发式即兴构建有序性；CFLT 提供一个确定性的单一规则。
...
**CFLT 映射（类比，非推导）：** DRT 提供的是关于话语表征*如何*被增量更新的词汇，但它**并未**推导出一种普遍的事件优先更新策略 —— Core-first 是 CFLT 的选择，而非 DRT 定理。借助 DRT 这一类比，CFLT *预测*：如果首先引入核心，会较早建立中心话语指称对象 —— 事件变量、身份主张、状态或言语行为 —— 从而使随后的修饰语附加到一个已经存在于表征中的指称对象上。CFLT 假设这能减少修饰语出现在其目标之前所产生的临时歧义；该收益的大小是需要测量的，而非 DRT 所确立的结论。
...
这是一种**算子堆叠（operator-stacking）**读取方式：$C$ 是最内层的确定；原因、地点和时间可读作包裹它的渐进“外部”模态-时间算子，将它们由核心向外线性化与协议语序**相容**。**这是一个起激励作用的类比，而非推导：**标准模态-时态逻辑并不为原因/地点/时间固定唯一的作用域顺序（它们的相对作用域随语言与构式而变），故具体的 R→S→T 序是从认知/加工层面、而非逻辑论证的，其地位仍是开放的经验问题（见 [`mathematics.md`](./mathematics.md) §12、[`linguistics.md`](./linguistics.md) §4.3；亦参上文 §7 关于 Manner 准则——它同样只激励“有一个序”，而不蕴含“正是这个序”）。
