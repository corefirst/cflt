...
CFLT 借鉴了这一趋同模式并将其应用于自然语言。核心（显著性锚点）被尽早确定；`[原因] → [地点] → [时间]` 修饰语随后附加。**其输出是自然语言**，而非形式化符号 —— 但其概念顺序与形式逻辑一个多世纪以来所使用的顺序是一致的。
...
**CFLT 映射：** 核心是一叠修饰函数中的**最内层（基础）项**，而修饰语则是包裹它的高阶函数。一个 CFLT 话语的语义结构可以建模为嵌套的应用，核心位于栈底：

$$\text{时间}\,(\,\text{地点}\,(\,\text{原因}\,(\,\text{核心}\,)\,)\,)$$

> **关于方向的说明：** 上述符号是*语义上的*嵌套（时间是最外层的包裹，核心是最内层的确定）。CFLT 的*表面*线性化是对这一栈结构的由内而外的读取 —— 首先说出核心，然后随着每个包裹的修饰语被“拆封”而依次附加。等效地，CFLT 的表面形式是通过对 $[\text{核心}, \text{原因}, \text{地点}, \text{时间}]$ 进行左折叠（left-fold）生成的，每一步都附加（append）下一个外部修饰语，而不是前置（prepend）它。Lambda 比喻抓住了*谁依赖谁*；线性化规则抓住了*什么时候说什么*。
