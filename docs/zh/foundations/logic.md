...
> **关于承诺类 (Commissives) 与四类压缩的说明。** 上述映射省略了 Searle 五类中的两类 —— **承诺类**（promises，如 *"我明天会给你打电话"*；*"我保证会来"*) 和 **宣告类**（上文提到的专门施事范畴）。在 CFLT 中，承诺类实现为**动作核心的一个子类**：谓词是一个承诺动作（*保证、承诺、确保、会做*），嵌入的命题内容是被承诺的动作 —— 因此 *"我保证会来"* 解析为核心 = *我保证会来*。宣告类，在窄义施事范畴之外出现时，是身份核心的一个有标子类（通常是带规定约束力的系词谓语）。因此 CFLT 的四分分类法是对 Searle 五类行为的**有意进行的教学/操作性聚合** —— 这与上文关于宣告类的注记一致 —— 并非含蓄地否认承诺类和宣告类的存在。记录这一压缩是为了防止读者寻找缺失的第五种核心类型。
