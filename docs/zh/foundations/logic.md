...
**CFLT 作为一种关联最大化策略：** 将核心置于位置 0，将最高效果的 Token 放在了听者注意力最集中的地方。听者（或 LLM，见 `llm.md`）可以从核心开始进行推理计算，而不需要在修饰语中漫长等待以发现话语的主题。（注意：关联理论**激励**前置高效果内容，但并不**蕴含**位置 0 本身——它权衡的是*总*认知效果与*总*努力，且允许后置信息也最优相关。所以关联理论与 Core-first **相容**，非**证明**。）
