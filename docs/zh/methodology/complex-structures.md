...
基础 CFLT 序列 —— `[核心动作/结果] → [条件/原因] → [空间/上下文] → [时间]` —— 针对单一、离散的思想进行了高度优化。它体现了“扁平化逻辑”。
...
**条件句的 CFLT-Complex 规则：** 始终先断言 **结果 (核心/Core)**，然后在 `[条件/原因]` 槽位中追加 **条件**。
...
对于 LLM 提示词 (Prompting) 和多 Agent 通信，在 `[原因]` 或 `[空间]` 槽位中传递大量上下文会破坏解析器的效率。
