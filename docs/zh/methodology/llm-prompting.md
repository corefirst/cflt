...
## 1. 问题所在：提示词方差与长上下文性能下降

自然语言具有高熵和高方差的特性。在生产级 AI 系统中，用户语序的微小变化可能导致模型行为的显著差异（即**顺序敏感性**问题）。此外，LLM 在位置 0 表现强**首因（Primacy）**、末尾**近因（Recency）**，往往会忽略埋在长提示词中间的关键信息。（注：Xiao 等 2024 确立的是"注意力汇点" —— 一种 softmax 稳定性副产物，初始 token 即使*语义关联很弱*也会累积高注意力。CFLT *假设*首因会在位置 0 额外贡献一种语义收益；这一联合分解是 CFLT 待检验的提案，而非汇点文献的结论。详细消歧参见 [`../foundations/llm.md`](../foundations/llm.md) §2.3。）

## 2. 解决方案：将 CFLT 协议作为 "线缆格式 (Wire Format)"

**CFLT 协议**充当了**结构稳定器**的作用。通过强制执行固定序列 (`[核心] → [理由] → [空间] → [时间]`)，你可以使模型的计算与其固有的位置偏差相对齐。

> **警示 —— 多语言 LLM 稳定性。** CFLT 设计为*语言无关协议* —— 其槽位语义与顺序主张意在适用于任何表面语言。然而当前 LLM 在不同语言上**能力不均衡**：实证工作（Lai et al. 2023 *ChatGPT Beyond English*；Bang et al. 2023；"Don't Trust ChatGPT when your Question is not in English"，EMNLP 2023）显示非英语输入上有显著退化，包括指令跟随、推理与安全。CFLT 无法消除这一 gap。CFLT *假设*它能减少跨语言转写中的*协议层*漂移，但这是一项需要多语言消融实验验证的待检验预测，而非所引文献已展示的效应。不要把 CFLT 设想的语言无关性解读为"用越南语或斯瓦希里语渲染的 CFLT 提示会像英语版一样表现"。

### 2.1 位置 0 的注意力
LLM 不成比例地关注前几个 token（位置 0）。Xiao 等 2024 记录了其中一个成分 —— **注意力汇点**，一种 softmax 稳定性副产物，初始 token 即使语义上不重要也会累积高注意力；他们*没有*证明置于早期的内容会因此更具语义影响力。CFLT *假设*存在第二个语义成分 —— **首因**（因果掩码使早期 token 的影响累积），并*预测*把**核心动作**置于高注意力前缀区会使模型的早期状态条件于主要意图。这一首因/汇点联合分解，以及它所蕴含的语义收益，是 CFLT 待检验的提案，而非汇点文献的既有结论。首因 vs 汇点的细致消歧参见 [`../foundations/llm.md`](../foundations/llm.md) §2.3。
...
### 4.3 RAG (检索增强生成) 命中效率（假设）
当用户的查询被传递给向量数据库进行 RAG 时，嵌入模型在整句上做池化。
- **CFLT 假设:** 通过将**核心动作**放在查询的开头，嵌入向量可能受实际 “意图” 的影响更强，受 “语境噪声” (时间/空间) 的影响较小。
- **状态:** 这是 CFLT 待检验的预测，*而非*既有结论。所引 RAG 文献（Lewis et al. 2020）并未改变查询成分的顺序，因此不提供 core-first 查询提升 Top-K 召回的证据。验证这一点需要一项专门针对检索器、直接测量 Top-K 准确率的 CFLT/control 基准。
...
### 5.1 已建立的行业基准 (经验性，通用)
现代推理框架 (如 vLLM 的 APC, SGLang 的 RadixAttention) 为提供固定、可重用提示词前缀的工作负载报告了性能提升。下方的数字是**各基准自身工作负载下的通用前缀缓存收益**，而非 CFLT 特有的结果，CFLT 也**不会**自动继承它们：CFLT 提示词能否、以及在多大程度上*符合*获得这些收益的条件，需要在 CFLT 与匹配的非 CFLT 模板上做工作负载层面的消融实验（见 §4.2 与 [`./evaluation-metrics.md`](./evaluation-metrics.md)）。

- **TTFT (首个令牌时间) 降低:** **SGLang (2024)** ([NeurIPS 2024](https://openreview.net/forum?id=VqkAKQibpq)) *在其评测的工作负载下*报告缓存前缀的 TTFT 最多可**降低 80%–95%**；幅度取决于共享前缀长度、缓存可用性、调度、引擎配置与基线。CFLT 不经自身测量不继承该区间。
- **吞吐量增益:** 在其测试的智能体推理和多轮工作负载中，**RadixAttention** 通过最大化前缀重用报告了比基线 vLLM **高出 2x–5x 的吞吐量** ([LMSYS Org. 2024](https://lmsys.org/blog/2024-01-17-sglang/))；该数字同样是工作负载特定的，并非 CFLT 结果。
- **线性化影响:** **DOVE 研究 (2025)** ([Findings of ACL 2025](https://doi.org/10.18653/v1/2025.findings-acl.611)) *联合*扰动了*多个*提示词维度（措辞、分隔符、选项枚举符、示例、选项顺序），并发现某些扰动在某些模型–数据集组合上产生超过十个百分点的绝对准确率差异。这证实了**提示词结构在实质上影响模型行为** —— 这是 CFLT 的设计选择产生影响的*必要前提*。它**没有**把信息顺序单独隔离为原因，DOVE 也未确立可归因于信息顺序的、在 MMLU 上 10%–15% 的差距；MMLU 本身不确立任何顺序效应。不要把 DOVE 读作 CFLT 特别捕捉到任何此类差距的证据。
