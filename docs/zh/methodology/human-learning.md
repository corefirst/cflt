...
CFLT 通过将语言视为“语义乐高”来解决这个问题 —— **不变的是核心优先（Core-first）**（说话者的承诺先行），而其余 —— 既包括核心的**内部组装**（格标记、助词、介词、核内语序），**也包括尾巴（场景框架）的排序** —— 都交由每种语言的原生习惯处理。我们以 *核心 → 理由 → 空间 → 时间* 作为**一种示例排法**；尾巴序本身是**语言相对的、开放的实证问题**（参见 [`empirical-agenda.md`](empirical-agenda.md) §1.2）。分层划分参见 [`../foundations/core-concept.md`](../foundations/core-concept.md) §2.3。
...
2.  **时间最后:** 在最后附加 “时间令牌”。
3.  **肌肉记忆训练:** 训练学习者将最后的时间令牌视为一个 "触发器"，追溯性地验证核心。CFLT *假设*：在 "语法叠加" 阶段，大脑开始通过潜意识反馈回路将时间令牌 *预回响 (pre-echo)* 到核心动词中，将 "I go... yesterday" 转化为 "I went... yesterday"。该"预回响"是一个有待检验的 CFLT 训练机制，而非已确立的发现；没有任何被引文献确立它能复现母语者处理长距离依赖关系的方式。其预期成果是在不增加规划成本的情况下建立词形变化的肌肉记忆。
...
CFLT 的有效性主张*受*成熟的解析原则和双语产出模型*启发*，并被操作化为下列预估指标。这些是有待检验的预测，而非已测量的结果；其中的数值量级是 CFLT 的项目目标，而非从被引文献推导出的效应量。
...
- **CFLT 预估:** CFLT 将 EIC 应用于话语层槽位是一种*类比*，而非推导；Hawkins 的指标定义于句法成分识别域之上，本身并不产出话语层槽位比率或工作记忆缩减系数。CFLT *假设*前置 Core 能降低工作记忆的 “前瞻” 负荷，§6 的延迟测试旨在测量其是否成立以及幅度多少。任何具体量级（例如接近 100% 的 EIC 比率，或 2×–5× 的负荷降低）都是有待测量的 CFLT 目标，而非由 EIC 蕴含的数值。

### 5.2 产出延迟 (实验预估)
**Hashimoto, Yokoyama & Kawashima 2012** [[10.2174/1874347101206010062](https://doi.org/10.2174/1874347101206010062)] 是一篇*文献综述*，比较了 SVO 与 SOV 语言句子理解研究所报告的激活模式；它报告了跨类型学差异（含额叶区域），并将其解读为与不同的加工与工作记忆负荷相容。它**并未**开展受控的非规范-vs-规范（或图形-背景反转）实验，因此它提供的是**间接的神经类型学动机**，而非"非规范词序引发左额叶活动增加"的直接测量。其与 CFLT 的相关性，我们视为有待一项主要 fMRI 研究去确定的开放问题。（Bemis & Pylkkänen 2013 与 Pylkkänen 2019 的 LATL 组合研究涉及最小语义组合；我们不引用它来支持"语义组合在不同词序下稳定"，该工作并未确立这一点。）
- **重构延迟:** 由于 “修饰语陷阱”，将 L1 (主题优先) 映射到 L2 (主语优先) 在自发言语中通常会增加 **200ms–500ms** 的认知延迟。
- **假设:** CFLT *预测*：训练能减少（在极限情况下或可消除）这一特定的重组延迟，方式是将线性化决策移至言语前的自动阶段。这是一项有待 §6 检验的 CFLT 专属实证预测。[Levelt (1989)](https://mitpress.mit.edu/9780262620895/speaking/) 的产出架构提供了 CFLT 定位该干预的阶段（概念器/形式器边界）；Levelt 本人并未预测任何支架能消除重组延迟。
