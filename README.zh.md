...
2. **[`docs/zh/foundations/core-concept.md`](./docs/zh/foundations/core-concept.md)** —— **在读完宣言后立即阅读**。定义 CFLT 中"核心"的含义（显著性锚点 —— 事件、状态、系属或指令 —— **不是**动词或谓语），并驳斥最常见的误读。同时探讨 CFLT 如何作为无标记默认值，而非唯一允许的形式。
...
4. **保持"核心"定义的一致性。** 核心 = 显著性锚点 —— **事件**（发生 / 做了某事）/ **状态**（处于某种情况）/ **系属**（X 是什么）/ **指令**（要对方做某事）。不是动词。不是谓语符号。
