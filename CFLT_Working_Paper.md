...
Second-language (L2) learners and Large Language Model (LLM) prompts share a common cost: turning a multidimensional thought into a single ordered string of words. Learners pay it as working-memory overhead; LLMs pay it as order-sensitivity and "Lost in the Middle" attention degradation. This paper presents Core-First Language Theory (CFLT), a normative discourse-level linearization protocol that fixes the relative order of an utterance as `[Core] → [Reason] → [Space] → [Time]`: the speaker's salience anchor (the event, identity, state, or request being committed to) comes first — not the verb, and not a judgment about "the most important word" — and the surrounding frame of reason, place, and time follows. Placing the Core first is designed to move the linearization decision out of the producer's real-time effort, so that the learner fills a fixed scaffold rather than restructuring an L1 plan into L2 syntax, and the model meets the task in its high-attention prefix rather than searching for it mid-prompt. CFLT stays one level above grammar — each language builds the Core in its own syntax, while the protocol governs only the order of the surrounding frame. A pilot study (24 cases × 2 conditions × 3 runs × 5 frontier models = 720 trials) provides preliminary evidence that Core-first prompts (i) raise extraction accuracy on the distractor-heavy condition (Level 3) from a mean of 65.6% to 100% across all five models, and (ii) cut completion tokens by up to 38% on reasoning models with visible chain-of-thought, with no token effect on short-output models. On a separate buried-decision condition (Level 4), one model (DeepSeek V4 Pro) shows no benefit (−11 pp) while the other four saturate at ceiling — a model-specific anomaly we report because it constrains the theory. We frame these results as suggestive, not confirmatory, and position CFLT as a language-neutral interface for human–AI reasoning whose falsification conditions are laid out as an empirical agenda. Reproducible extraction logic, evaluation scripts, and the raw 720-trial logs are available at https://github.com/corefirst/cflt.
...
The primacy advantage is nonetheless task-dependent, not a universal first-position favoritism. Pezeshkpour & Hruschka (2024) find that multiple-choice accuracy shifts by tens of percentage points with option order, the *direction* of the bias differing across models; Berglund et al. (2024, "The Reversal Curse") show that order-sensitivity is entangled with the learned representations themselves, not just with prompt-time formatting. Two consequences for CFLT follow: (a) the buried-decision condition (L4) can interact with model-specific properties rather than uniformly benefiting from Slot-0 placement — one model regresses while the others saturate, reported and analyzed in §6.5; and (b) the stronger *CFLT-as-alignment-constraint* upgrade (§7.3) is harder than CFLT-as-prompt-protocol, because the Reversal Curse implies training-time exposure may not yield inference-time robustness.^[A narrower scope note: prompt-order sensitivity (Lu et al. 2022; Sclar et al. 2024) and Min et al.'s (2022) partial counter-result — demonstration *content* matters surprisingly little for in-context learning while *format and order* are first-order variables — place CFLT's contribution on the format-and-order axis, not on content correctness. This is part of the contribution, not a defect: fixing both the schema and the linear order of the speaker's commitment is the joint discipline.]
...
| DeepSeek V4 Pro | L3, L4 | 69% | 86% | **+16.7 pp** (L3 alone: +44pp; L4 −11pp regression drags aggregate, see §6.5) |
...
- **Claude L2 noise observation.** Claude Sonnet 4.6 is the only model where L2 showed a non-zero aggregate (83% → 78%, −6 pp), but per-case inspection shows this is sampling noise, not a stable regression: strong single-case positives and negatives mostly cancel, and at N=3 runs per arm the −6 pp sits within sampling noise.^[Per case: ZH_L2_01 went 0% → 100% (+100 pp), ZH_L2_02 went 100% → 0% (−100 pp), EN_L2_03 showed −33 pp; the remaining three L2 cases were saturated on both arms. The −6 pp aggregate is the algebraic residual after these cancel.] A confirmatory study with N ≥ 10 per case is required to resolve the direction.
- **L4 null/negative result.** Of the five models, only DeepSeek V4 Pro shows an informative L4 regression (−11 pp); the other four are at or near ceiling on L4 under both arms (GPT-5 100%/100%, Gemini 94%/94%, Qwen 94%/92%, Claude 100%/100%). The reading most consistent with the data is a DeepSeek-specific model × item-type interaction, not a general primacy failure (four of five models show no L4 issue). To keep that from sliding into ad-hoc rescue, we pre-commit anti-rescue criteria for the confirmatory study: the redesigned L4 set must hold embedding-distance between the buried decision and the distractors above a pre-registered threshold, syntactically anchor the decision (e.g., "我决定", "I decide") rather than merely position it last, and be re-authored by ≥ 2 independent annotators — if the regression survives all three it is locked in as a model × item interaction, and if it disappears under any it is refuted.^[Three readings were weighed against the cross-model evidence. (a) *Primacy over-strong as a unified explanation — weakly supported*: it predicts the L4 regression should recur wherever L4 is informative, yet four of five models show none; a weaker surviving form (primacy locally overridden when the natural narrative order is itself a strong reasoning scaffold) rests on N=1 model. (b) *DeepSeek-specific model × item interaction — most consistent with the data* (guarded by the anti-rescue criteria above). (c) *Reasoning models compensate via internal search — not supported*: it predicts the regression should track reasoning-trace length, but Qwen (longest traces) shows L4 noise (−3 pp), DeepSeek (shorter traces) shows −11 pp, and Claude (no visible trace) shows zero; not refuted given N=5, but deprioritized.] The primacy mechanism (§3.2) stays consistent with the five-model L3 result and the four-of-five L4 saturation; operationally, deployments on DeepSeek for multi-option decisions should A/B-test CFLT before committing.
- **Model coverage.** Five models across five commercial providers is the cross-provider base for the L3 universality claim; the four-of-five L4 saturation result is also drawn from this set. Stronger generalization claims (and tighter resolution of the DeepSeek L4 anomaly) require ≥ 8 frontier models including at least one from each major architecture family, with a planned re-run when checkpoints update.
...
Methodology. Within-subjects design with CFLT-trained vs. untrained translation conditions; measure BOLD signal in dorsolateral PFC and the dorsal anterior cingulate (Salience Network nodes, Seeley et al. 2007); align with the word-order-processing paradigm reported by Hashimoto, Yokoyama & Kawashima (2012), whose preliminary findings on cross-linguistic word-order modulation of brain response were presented as a conference abstract by the same lab in 2006 (Yokoyama, Fukushima, Riera & Kawashima 2006, *NeuroImage* OHBM abstract).^[The 2012 paper is published in *The Open Medical Imaging Journal* (Bentham Open); the methodology is standard fMRI block design and the imaging protocol is replicable, but the venue has weaker peer-review scrutiny than the canonical neuroimaging journals. The prudent reading is to treat Hashimoto et al. 2012 as a *methodological reference*, not an evidential anchor on its own, and to require the present study's contrast to replicate against an independent imaging cohort before drawing strong conclusions.]
